(function ($) {
    ...
    function getOption(objId) {
        return {
            ...
            thumb: {width: 100, height: 100, quality: 100, allowMagnify: false, crop: true, type: "image/jpeg"},
            method: "POST",
            server: "",
            sendAsBinary: false,
            chunked: true,
            chunkSize: 5120 * 10240,
            fileNumLimit: 500,
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        if ($parentFileBox.find('.diyButton').length <= 0) {
            var div = '<div class="diyButton"> \
						<button class="diyStart btn btn-info">开始上传</button> \
						<button class="diyCancelAll btn btn-warning">全部取消</button> \
					</div>';
